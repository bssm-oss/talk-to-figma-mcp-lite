...
// src/talk_to_figma_mcp/lite.ts
function liteOk(data, extras = {}) {
  return { ok: true, data, ...extras };
}
function liteFail(code, message, recoverable, nextAction) {
  return {
    ok: false,
    error: { code, message, recoverable, nextAction }
  };
}
function liteTextResponse(response) {
  return {
    content: [
      {
        type: "text",
...
function liteErrorFromUnknown(error, nextAction) {
  return liteFail(
...
function liteCreateNodesResult(created, warnings, affectedNodeIds) {
...
function planInspectDesign(input = {}) {
...
function planUpdateNodes(input) {
  ...
  for (const patch of input.patches) {
    ...
    const dispatches = [];
    if (patch.geometry?.x !== void 0 || patch.geometry?.y !== void 0) {
      if (patch.geometry.x === void 0 || patch.geometry.y === void 0) {
    ...
    if (patch.geometry?.width !== void 0 || patch.geometry?.height !== void 0) {
      if (patch.geometry.width === void 0 || patch.geometry.height === void 0) {
    ...
    if (patch.style?.stroke) {
      if (patch.style.stroke.weight === void 0) {
    ...
    if (patch.layout?.mode !== void 0 || patch.layout?.wrap !== void 0) {
      if (patch.layout.mode === void 0 || patch.layout.wrap === void 0) {
    ...
    if (patch.layout?.align) {
      ...
      if (primary === void 0 || counter === void 0) {
...
function planViewAndExport(input) {
...
function planCreateNode(node, parentId) {
  if (node.kind === "frame") {
    return {
...
function describePayload(value) {
  ...
  const params = message?.params && typeof message.params === "object" ? message.params : void 0;
  return [
...
var DEFAULT_FIGMA_PORT = 3055;
var liteSessionSchema = {
  action: import_zod.z.enum(["status", "join", "reconnect"]),
  channel: import_zod.z.string().optional()
};
var inspectTargetSchema = import_zod.z.union([
  import_zod.z.object({ kind: import_zod.z.literal("document") }),
  import_zod.z.object({ kind: import_zod.z.literal("selection") }),
  import_zod.z.object({ kind: import_zod.z.literal("nodes"), nodeIds: import_zod.z.array(import_zod.z.string()) }),
  import_zod.z.object({ kind: import_zod.z.literal("subtree"), nodeId: import_zod.z.string() }),
  import_zod.z.object({ kind: import_zod.z.literal("assets") })
]);
var inspectDesignSchema = {
  target: inspectTargetSchema.optional(),
  detail: import_zod.z.enum(["summary", "structure", "full", "text", "types"]).optional(),
  types: import_zod.z.array(import_zod.z.string()).optional(),
  maxDepth: import_zod.z.number().optional(),
  chunkSize: import_zod.z.number().int().min(1).max(100).optional()
};
var rgbaSchema = import_zod.z.object({
...
var createNodeSchema = import_zod.z.union([
  import_zod.z.object({
    kind: import_zod.z.literal("frame"),
    ...
    layoutWrap: import_zod.z.enum(["NO_WRAP", "WRAP"]).optional(),
    paddingTop: import_zod.z.number().min(0).optional(),
    paddingRight: import_zod.z.number().min(0).optional(),
    paddingBottom: import_zod.z.number().min(0).optional(),
    paddingLeft: import_zod.z.number().min(0).optional(),
    ...
    itemSpacing: import_zod.z.number().min(0).optional()
  }),
  import_zod.z.object({
    kind: import_zod.z.literal("rectangle"),
    name: import_zod.z.string().optional(),
    x: import_zod.z.number().optional(),
    y: import_zod.z.number().optional(),
    width: import_zod.z.number().positive(),
  ...
  import_zod.z.object({
    kind: import_zod.z.literal("text"),
    name: import_zod.z.string().optional(),
    x: import_zod.z.number().optional(),
    y: import_zod.z.number().optional(),
    text: import_zod.z.string(),
    fontSize: import_zod.z.number().positive().optional(),
    fontWeight: import_zod.z.number().optional(),
    fontColor: rgbaSchema.optional()
  })
]);
var createNodesSchema = {
  parentId: import_zod.z.string().optional(),
  nodes: import_zod.z.array(createNodeSchema).min(1),
  selectCreated: import_zod.z.boolean().optional()
};
var stylePatchSchema = import_zod.z.object({
  fill: rgbaSchema.optional(),
  stroke: import_zod.z.object({ color: rgbaSchema, weight: import_zod.z.number().positive().optional() }).optional(),
  cornerRadius: import_zod.z.object({
    radius: import_zod.z.number().min(0),
    corners: import_zod.z.tuple([import_zod.z.boolean(), import_zod.z.boolean(), import_zod.z.boolean(), import_zod.z.boolean()]).optional()
  }).optional()
});
var layoutPatchSchema = import_zod.z.object({
  mode: import_zod.z.enum(["NONE", "HORIZONTAL", "VERTICAL"]).optional(),
  wrap: import_zod.z.enum(["NO_WRAP", "WRAP"]).optional(),
  padding: import_zod.z.object({
    top: import_zod.z.number().min(0).optional(),
    right: import_zod.z.number().min(0).optional(),
    bottom: import_zod.z.number().min(0).optional(),
    left: import_zod.z.number().min(0).optional()
  }).optional(),
  align: import_zod.z.object({
    primary: import_zod.z.enum(["MIN", "MAX", "CENTER", "SPACE_BETWEEN"]).optional(),
    counter: import_zod.z.enum(["MIN", "MAX", "CENTER", "BASELINE"]).optional()
  }).optional(),
  sizing: import_zod.z.object({
    horizontal: import_zod.z.enum(["FIXED", "HUG", "FILL"]).optional(),
    vertical: import_zod.z.enum(["FIXED", "HUG", "FILL"]).optional()
  }).optional(),
  spacing: import_zod.z.object({
    item: import_zod.z.number().min(0).optional(),
    counterAxis: import_zod.z.number().min(0).optional()
  }).optional()
});
var updateNodesSchema = {
  mode: import_zod.z.enum(["preview", "apply"]),
  confirmDestructive: import_zod.z.boolean().optional(),
  patches: import_zod.z.array(import_zod.z.object({
    nodeId: import_zod.z.string(),
    rename: import_zod.z.string().optional(),
    geometry: import_zod.z.object({
      x: import_zod.z.number().optional(),
      y: import_zod.z.number().optional(),
    ...
    text: import_zod.z.object({ characters: import_zod.z.string(), preserveStyle: import_zod.z.boolean().optional() }).optional(),
    clone: import_zod.z.object({ x: import_zod.z.number().optional(), y: import_zod.z.number().optional() }).optional(),
    delete: import_zod.z.boolean().optional()
  })).min(1)
};
var manageTextSchema = {
  action: import_zod.z.enum(["scan", "replace"]),
  scope: import_zod.z.object({ nodeId: import_zod.z.string().optional(), selection: import_zod.z.boolean().optional() }),
  replacements: import_zod.z.array(import_zod.z.object({ nodeId: import_zod.z.string(), text: import_zod.z.string() })).optional(),
  preserveStyle: import_zod.z.boolean().optional(),
  mode: import_zod.z.enum(["preview", "apply"]).optional(),
  chunkSize: import_zod.z.number().int().min(1).max(100).optional()
};
var viewAndExportSchema = {
  action: import_zod.z.enum(["focus", "select", "export"]),
  nodeIds: import_zod.z.array(import_zod.z.string()).optional(),
  nodeId: import_zod.z.string().optional(),
  export: import_zod.z.object({
    format: import_zod.z.literal("PNG").optional(),
    scale: import_zod.z.number().positive().max(10).optional()
  }).optional()
};
...
server.tool(
  ...
  async (input) => {
    try {
      ...
      const result = await sendCommandToFigma(plan.command, plan.params ?? {});
      const affectedNodeIds = input.nodeIds ?? (input.nodeId ? [input.nodeId] : void 0);
      return liteTextResponse(liteOk(result, { affectedNodeIds }));
    } catch (error) {
...
server.tool(
  ...
  "Create a new frame in Figma",
  {
    ...
    paddingTop: import_zod.z.number().min(0).optional().describe("Top padding for auto-layout frame"),
    paddingRight: import_zod.z.number().min(0).optional().describe("Right padding for auto-layout frame"),
    paddingBottom: import_zod.z.number().min(0).optional().describe("Bottom padding for auto-layout frame"),
    paddingLeft: import_zod.z.number().min(0).optional().describe("Left padding for auto-layout frame"),
    ...
    itemSpacing: import_zod.z.number().min(0).optional().describe("Distance between children in auto-layout frame. Note: This value will be ignored if primaryAxisAlignItems is set to SPACE_BETWEEN.")
  },
  async ({
    x,
    y,
...
server.tool(
  ...
  "Delete a node from Figma",
  {
    ...
    confirmDestructive: import_zod.z.boolean().optional().describe("Must be true to confirm destructive deletion")
  },
  async ({ nodeId, confirmDestructive }) => {
    try {
...
server.tool(
  ...
  async ({ nodeIds, confirmDestructive }) => {
    try {
      if (confirmDestructive !== true) {
        return {
          content: [
...
server.tool(
  ...
  "Export a node as an image from Figma",
  {
    ...
    format: import_zod.z.literal("PNG").optional().describe("Export format"),
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the frame to modify"),
    paddingTop: import_zod.z.number().min(0).optional().describe("Top padding value"),
    paddingRight: import_zod.z.number().min(0).optional().describe("Right padding value"),
    paddingBottom: import_zod.z.number().min(0).optional().describe("Bottom padding value"),
    paddingLeft: import_zod.z.number().min(0).optional().describe("Left padding value")
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the frame to modify"),
    itemSpacing: import_zod.z.number().min(0).optional().describe("Distance between children. Note: This value will be ignored if primaryAxisAlignItems is set to SPACE_BETWEEN."),
    counterAxisSpacing: import_zod.z.number().min(0).optional().describe("Distance between wrapped rows/columns. Only works when layoutWrap is set to WRAP.")
  },
...
function connectToFigma(port = 3055) {
  ...
  ws.on("open", () => {
    ...
    currentChannel = null;
    if (desiredChannel) {
      sendCommandToFigma("join", { channel: desiredChannel }).then(() => {
  ...
  ws.on("message", (data) => {
    try {
      ...
      if (myResponse.id && pendingRequests.has(myResponse.id) && ("result" in myResponse || "error" in myResponse)) {
